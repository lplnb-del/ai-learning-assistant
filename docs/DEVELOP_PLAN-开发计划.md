...
目标：形成学习产品差异化能力，把问答沉淀为可管理、可复习、可参与 RAG 的问答资产层，并实现知识库与问答库的双向打通。
...
- [x] 知识库与问答库打通：正向生成（知识库→问答卡片）+ RAG 混合检索。
- [x] 支持从问答卡片反向追溯到知识库来源切片。
- [x] 复习策略：三级掌握程度 + 按掌握程度/时间筛选排序（不做间隔重复算法）。
- [x] 支持多知识库联合检索，可同时选择多个知识库跨库检索。
